...
mod tests {
    ...
    fn test_new_buffer_is_empty() {
        let buffer = AudioRingBuffer::new(1000, 100);
    ...
    fn test_pop_returns_silence_before_min_fill() {
        let mut buffer = AudioRingBuffer::new(1000, 100);
    ...
    fn test_buffer_max_size_enforcement() {
        let mut buffer = AudioRingBuffer::new(10, 5);
    ...
    fn test_buffer_drops_oldest_when_full() {
        let mut buffer = AudioRingBuffer::new(5, 2);
        ...
        // Add one more - should drop the oldest (1.0)
        buffer.push(6.0);

        assert_eq!(buffer.len(), 5);
        // Pop samples - first should be 2.0 (1.0 was dropped)
        assert_eq!(buffer.pop(), 2.0);
    }

    #[test]
    fn test_pop_empty_buffer_returns_zero() {
        let mut buffer = AudioRingBuffer::new(1000, 0); // min_fill = 0
        assert_eq!(buffer.pop(), 0.0);
    }

    #[test]
    fn test_fifo_ordering() {
        let mut buffer = AudioRingBuffer::new(1000, 0);
        ...
        assert_eq!(buffer.pop(), 2.0);
        assert_eq!(buffer.pop(), 3.0);
        assert_eq!(buffer.pop(), 4.0);
    }
}
